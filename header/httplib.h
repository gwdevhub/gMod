...
namespace httplib {
    ...
    inline uint64_t Result::get_request_header_value_u64(const std::string& key,
                                                         size_t id) const
    {
        return get_header_value_u64(request_headers_, key, id, 0);
    }
    ...
    namespace detail {
        ...
        inline size_t to_utf8(int code, char* buff)
        {
            ...
            if (code < 0xE000) { // D800 - DFFF is invalid...
                return 0;
            }
            if (code < 0x10000) {
                buff[0] = static_cast<char>(0xE0 | ((code >> 12) & 0xF));
        ...
        private:
            ...
            static constexpr size_t read_buff_size_ = 1024 * 4;
        };
        ...
        inline bool keep_alive(socket_t sock, time_t keep_alive_timeout_sec)
        {
            ...
            while (true) {
                const auto val = select_read(sock, 0, 10000);
                if (val < 0) {
                    return false;
                }
                if (val == 0) {
...
#define USE_IF2IP
            ...
            time_t write_timeout_usec, const std::string& intf, Error& error)
        {
            const auto sock = create_socket(
                ...
                [&](socket_t sock2, struct addrinfo& ai) -> bool {
                    ...
                    {
#ifdef _WIN32
                        const auto timeout = static_cast<uint32_t>(read_timeout_sec * 1000 +
...
#ifdef _WIN32
                        const auto timeout = static_cast<uint32_t>(write_timeout_sec * 1000 +
                              ...
                              const T& is_shutting_down, U& compressor, Error& error)
        {
            ...
            data_sink.done = [&]() { done_with_trailer(nullptr); };
        ...
        inline bool parse_range_header(const std::string& s, Ranges& ranges) try {
            ...
            if (std::regex_match(s, m, re_first_range)) {
                ...
                const auto len = static_cast<size_t>(m.length(1));
                                    ...
                                    size_t index)
        {
            ...
            const auto slen = static_cast<ssize_t>(content_length);
                                           ...
                                           Content content)
        {
            for (size_t i = 0; i < req.ranges.size(); i++) {
                ...
                const auto offsets = get_range_offset_and_length(req, res.content_length_, i);
    ...
    namespace detail {
        ...
        inline ssize_t SocketStream::read(char* ptr, size_t size)
        {
            ...
            if (size < read_buff_size_) {
                const auto n = read_socket(sock_, read_buff_.data(), read_buff_size_,
    ...
    inline Server& Server::set_header_writer(
        const std::function<ssize_t(Stream&, Headers&)>& writer)
    {
                                        ...
                                        const std::string& content_type)
    {
        ...
        if (res.content_length_ > 0) {
            ...
            if (req.ranges.size() == 1) {
                ...
                const auto offset = offsets.first;
                const auto length = offsets.second;
...
#endif
                const auto val = detail::select_read(svr_sock_, idle_interval_sec_,
                                                     idle_interval_usec_);
                if (val == 0) { // Timeout
                    task_queue->on_idle();
                    continue;
        ...
        if (res.body.empty()) {
            if (res.content_length_ > 0) {
                ...
                if (req.ranges.empty()) {
                    ...
                    const auto content_range = detail::make_content_range_header_field(
    ...
    {
        const auto ret = detail::process_server_socket(
                                            ...
                                            Error& error)
    {
        ...
        if ((res.status != 204) && req.method != "HEAD" && req.method != "CONNECT") {
            const auto redirect = 300 < res.status && res.status < 400 && follow_location_;
